...
This script only starts the Django frontend, without any dependencies on psutil.
"""
import os
import sys
import subprocess
...
def main():
    ...
    if not available:
        logger.warning("Port 8000 is already in use. The server might fail to start.")
        logger.warning("If the server fails to start, please close any application using port 8000 and try again.")
